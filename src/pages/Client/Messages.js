...
const chatRoomsList = (chat_rooms, current) => {
  ...
  return chat_rooms.map((chatRoom, index) => {
    ...
    return (
      ...
      >
        <Link
          to={`/client-dash/messages?room=${chatRoom._id}`}
          style={{ textDecoration: "none" }}
        >
          <div className="message-avatar">
            {chatRoom.members[1].profile ? (
              <img
                src={`${process.env.REACT_APP_API_BASE_URL}/${chatRoom.members[1].profile.profile_image.path}`}
                alt=""
              />
            ) : (
